...
def get_profit(duration,profits,installed,time_req,max_sales,holding_cost,max_inventory,store_target,hours_per_month):
    ...
    # initial balance
    balance0 = model.addConstrs(
        (produce[months[0], product] == sell[months[0], product] + store[months[0], product] for product in products),
        name="Initial Balance")

    # balance
    balance = model.addConstrs((store[months[months.index(month) - 1], product] + produce[month, product] == sell[
        month, product] + store[month, product] for product in products for month in months if month != months[0]),
                               name="Balance")

    # inventory target at the end of the last month
    inventory = model.addConstrs((store[months[-1], product] == store_target for product in products),
                                 name="Inventory_Target")

    # machine capacity
    capacity = model.addConstrs((gp.quicksum(
        time_req[machine][product] * produce[month, product] for product in time_req[machine]) <= hours_per_month *
                                 installed[machine] for machine in machines for month in months), name="Capacity")

    # Objective Function
    objective = gp.quicksum(
        profit[product] * sell[month, product] - holding_cost * store[month, product] for month in months for product in
        products)
